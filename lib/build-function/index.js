...
var exec = function(command, options) {
    return new Promise(function(resolve, reject) {
        child_process.exec(command, options, function(err, sdout, stderr) {
            if (err) return reject(err);
            resolve();
        });
    });
...
exports.handle = function(event, ctx, cb) {
    ...
    return mkdir(tmpDir).then(function() {
        ...
        // (3)
        //
        var cmdInstall = 'npm_config_cache=' + tmpCacheDir + ' npm install --production';
        ...
        // (6)
        //
        return exec('rm -rf ' + tmpDir + ' ' + tmpCacheDir);
    }).then(function() {
